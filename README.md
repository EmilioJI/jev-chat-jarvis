...
**小书童系列的聊天「对话副驾」：以悬浮入口接收你主动提供的聊天内容，给出判断与三条候选回复；微信正式模式以复制/粘贴为主，发送始终由你完成。**

旧采集架构曾在 **微信、QQ、X（Twitter 私信）** 做过真机验证，飞书采集也已接入。当前微信已迁移为“主动安全模式”，需要按剪贴板 / 可选本地 OCR 的新路径重新完成 A/B 真机回归。
...
- **一套分析内核，多种安全采集方式。** 微信采用主动提供内容的模式；QQ / X / 飞书可使用标准 Android 无障碍适配。判断、历史时效、候选生成与排序全部复用。
- **微信正式模式不读内部控件。** 不 Hook、不 Root、不改包、不走微信私有协议、不订阅微信 Accessibility 事件，也不使用微信资源 ID。内容由用户主动通过剪贴板或可选的本地 OCR 提供。
...
| 平台 | 状态 | 采集方式 | 备注 |
|---|---|---|---|
| 微信 Android A / B | ✅ 主动安全模式 | 标准悬浮窗 + 剪贴板；本地 OCR 可选 | 不订阅微信 Accessibility 事件，不读微信资源 ID；vivo 分身同样有悬浮入口 |
...
**3. 开权限。** 按主页向导开三项：
- 无障碍（仅用于用户同意的标准 Android 能力；微信本身不订阅 Accessibility 事件。本地截屏 OCR 需要系统允许该能力）
- 悬浮窗 / 显示在其他应用上层（展示分析）
...
QQ / X / 飞书 ──(标准无障碍，默认不自动分析)──▶ 可见内容
   ...
   意图 / 危险 / 需求 / 动作 / 该不该回          │
              └───────────────────┬───────────────────┘
                        ...
                        判断引擎给 3 条候选排序
                                  ▼
                半透明悬浮窗展示 → 复制回复 → 用户自行粘贴并发送
```

- **采集**：微信不走内部节点适配器，采用剪贴板优先、用户主动本地 OCR 可选；QQ / X / 飞书保留标准 Android 节点适配器。自动分析和自动 OCR 默认关闭。
...
3. 判断、候选、历史时效和悬浮窗复用；输出默认复制，后续正式“直接输入”计划由标准 Android IME 完成。
...
| 微信 | 内部节点不作为正式接口 | 不订阅微信 Accessibility 事件，不使用微信资源 ID；剪贴板优先，本地 OCR 按用户操作可选 |
...
- **微信不再依赖内部 Accessibility Tree。** 这样会牺牲自动获取备注名、自动历史抓取与直接填入，但显著降低对微信内部 UI 和版本细节的依赖。
...
- `app/` — Android 应用（Kotlin，传统 View）
  - `capture/` 主动/标准采集：微信使用主动输入，QQ/X/飞书使用受限标准适配；`ocr/` 提供用户主动的本地离线识别
